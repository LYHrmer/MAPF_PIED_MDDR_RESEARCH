...
2026-09-24 实测更新：完整固定查询仍未通过，中心准备阶段耗尽原供给。此次把业务来源与真实费用贯通：setup的98.937448%扣费发生在分配扫描，77次16字节申请来自首个几何对象的数值初构、返回移动及多层复制；未进入此前怀疑的FLINT批量冷分支。观察版与原版92条业务记录逐字相同，86条费用段守恒。主线下一步有明确目标：保持精确数值与所有权语义，验证减少几何中间复制的实现；先前对齐特化仍撤回。本轮没有新优化收益，更没有SRDC净收益结论，见Q2 §62。
