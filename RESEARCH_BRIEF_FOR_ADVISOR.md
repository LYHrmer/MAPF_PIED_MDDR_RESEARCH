...
2026-09-24 最新实测：减少几何外层复制的隔离候选通过8954项原生差分检查，实际复测使首个Geometry的第三cell从供给行98提前到87、窗口末构造位置更深。这是局部推进改善；完整查询仍未通过，中心准备仍耗尽原53,477,376步，扫描占99.012240%。观察版与候选plain的92条业务记录逐字一致、86条费用段守恒。原生产实现保持，候选未作为完整修复合入。窄核验又发现当前冻结合同明确逐候选扫描，不能直接跳过占用块；下一步应评估必要成本与表示方式，不把小改进外推为可负担，更没有SRDC净收益结论，见Q2 §63。
