...
2026-09-24 实测更新：完整固定查询已实际首跑，尚未通过。中心准备阶段耗尽原供给，失败前缀核账有效；原版及候选均未触发此前怀疑的FLINT批量冷分支。一个语义等价的对齐优化虽通过460800次有限差分，却在同预算内推进更少，已撤回。当前先解决准备阶段重复小分配与可负担性，再验证完整查询和小规模公平对照；尚无SRDC净收益结果。独立Astra Ultra评估把“固定空间误差安全层上的执行占用代价学习”列为潜在备选，但目前不据此更换PIE-D主线。后文原生机制结果仍保持原范围。
