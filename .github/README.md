...
- [73 主稿](../73_PIE_SPATIAL_EVIDENCE_TERMINAL_HANDOFF_AND_PAID_SERVICE_PREEXPERIMENT_DRAFT_20260908.md)：DRAFT / NOT-FIXED / NOT-REVIEWED / NOT-ACCEPTED。当前 SHA256 `68eecae75a83ff3fe2d90c73866bc4980eaffd395f323b7505c1609caf1706a3`，297583 bytes / 925 LF；文件名日期不代表当前内容版本。
...
| 当前必须收口的设计项 | 最小可验收交付与边界 |
|---|---|
| S5/S6 可信消息认证 | 明确采用哪一种认证语义及创建/验证/重传/保留/回收费用。现有可信模型可采用writer专属不可变消息和受控不透明能力句柄、逐字段收费绑定检查；该路线尚待正式写入主稿，不假称已实现密码签名或已选定新算法。 |
| S6 封闭参考例程接口 | 一张固定版本/输入表示、语义后条件、请求与元素工作界、具名失败和源调用映射的表，复用当前GNU11/Boost与arena证据；未知回调不获资格。已知UB的纸面修订保留，具体后端逐步计量符合性留实现资格，不重开全库审计。 |
| S3/S5/S6 首轮非空性 | 将上述有限例程合同、同一A的请求/存储/扫描费用、有限阶段/字段身份、deadline、交付及实体服务/Stop条件组成一个符号结构族的联合相容性证明。只要求至少一次真实服务，不要求D先胜R/E0；条件定理与联合可满足仍须区分。 |

这三项完成后固定自给候选，进入下述正式顺序审查；审查发现的实际矛盾仍须修正。§13净收益/信息机制保留可证伪条件或假说，效果由后续实验回答。完整roster/连续WORLD实例、PIE-D/H19实际复现资格以及ABI/后端重放仍是全实验前目标的必要后续工作，已定义接口的未实现状态不单独阻止固定设计候选。S7只保留版本覆盖和再分发范围，前后研究用途口径已统一；不重做L23/同目录检索或新增逐hash许可。满足相应准入后，Opus承担明确范围的实现/修复，根验证整合，关键理论交Astra Ultra。
