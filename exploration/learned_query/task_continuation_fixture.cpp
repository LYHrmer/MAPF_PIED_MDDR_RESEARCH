// Finite native task-service successor. All old fixtures remain unchanged.
...
R duration_prior(const R& length) { return O::sqrt(O::mul(rat(3), length)); }
...
unsigned completion_value(const I::State& s, const PublicPlans& plans,
    ...
    for (const auto& d : s.demands) {
        ...
        for (const auto& r : d.second->relations) {
            ...
            R release = O::add(duration_prior(a.geometry.length()), end_delivery_prior());
        ...
        for (std::size_t k = 0; k < lengths.size(); ++k) {
            arrival = O::add(arrival, duration_prior(lengths[k]));
...
std::string choose_probe(const P::View& view, const PublicPlans& plans, TaskPolicy rule,
    ...
    auto consider = [&](const std::vector<std::string>& sequence) {
        const unsigned v = completion_value(state, plans, sequence, opportunities, at, window);
...
struct Moving {
    ...
    Moving(const std::string& id, const std::string& owner, const G& g, const R& at, int n)
        : key(id), agent(owner), geometry(g),
          controller({rat(1),rat(2),rat(6),rat(1)},g.length(),at,g.length(),rat(-1),id), lane(n) {
...
class TaskWorld {
    ...
    void start_ready() {
        for (unsigned n=0; n<lanes.size(); ++n) {
            ...
            moves.emplace_back(new Moving(leg.key,l.agent,g,clock,static_cast<int>(n)));
    ...
    void query_opportunity(TaskPolicy rule, unsigned step) {
        ...
        if (rule == TaskPolicy::Srdc || rule == TaskPolicy::RR) {
            ...
            key=choose_probe(before,public_plans(),rule,opportunities,clock);
...
public:
    explicit TaskWorld(TaskPolicy policy) : p(task_initial(policy)),
        lanes{Lane("D1",1,-2,4),Lane("D2",2,-2,4),Lane("D3",1,6,10)} {
        for (const std::string key : {"A","B","C"}) moves.emplace_back(new Moving(key,"agent-"+key,blocker(key),rat(0),-1));
...
void task_comparison() {
    ...
    for (auto policy : {TaskPolicy::Srdc,TaskPolicy::RR,TaskPolicy::AdmissionOne,TaskPolicy::AdmissionPair,
                       TaskPolicy::CompletionOne,TaskPolicy::CompletionPair,TaskPolicy::CompletionPairLong,TaskPolicy::Wait}) {
        TaskWorld w(policy); results.push_back(w.run(policy));
    }
    check(results[0].queries==results[1].queries && results[1].queries==results[3].queries,"baseline sequence mismatch");
    check(results[5].queries==std::vector<std::string>{"C"},"short rolling completion failed to select C then WAIT");
    check(results[6].queries==std::vector<std::string>({"A","B"}),"long rolling completion failed to select AB");
    check(results[5].curve.front()>results[0].curve.front() && results[5].curve[1]<results[0].curve[1],"task curve lost early ordering reversal");
    check(results[5].served_before_ready>0,"task service was incorrectly gated by central READY");
    check(results.back().queries.empty() && results.back().curve.back()==9,"unqueried blockers never delivered their END");
    std::cout << "{\"event\":\"summary\",\"status\":\"passed\",\"checks\":" << checks
              << ",\"finite_task_service\":true,\"production_AUTH\":false,\"full_paid_cost\":false"
              << ",\"learning\":false,\"lifelong_performance_claim\":false}\n";
}
} // namespace
int main() {
