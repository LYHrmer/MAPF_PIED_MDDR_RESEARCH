"""Run the finite native task-service fixture; preserve failures and old evidence."""
import argparse
...
FIXTURE = HERE / "task_continuation_fixture.cpp"
PROTECTED_NAMES = (
...
def run(source_root, output):
    ...
    report = dict(
        ...
        scope="artificial_task_service_and_continuation", source_root=str(root),
        source_header_sha256=pins, protected_sha256=protected,
        production_AUTH=False, full_paid_cost=False, learning=False,
    ...
    with output.open("x", encoding="utf-8") as stream:
        try:
            ...
            pins.update(loaded)
            with tempfile.TemporaryDirectory(prefix="mapf_task_continuation_") as directory:
                ...
                binary = build / "task_continuation"
                compile_command = [
                ...
                report["finite_task_service"] = events[-1].get("finite_task_service") is True
...
def main():
    ...
    if (output.exists() or output.is_symlink() or output.resolve().parent != HERE
            or not output.name.startswith("task_continuation_run_") or output.suffix != ".json"):
        parser.error("evidence must be a new task_continuation_run_*.json file in this package")
    ...
    print(json.dumps({
        ...
        "results": [event for event in report.get("events", [])
                    if event.get("event") in ("policy_result", "summary")],
    }, indent=2))
