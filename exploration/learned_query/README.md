...
创建及更新：2026-09-24。状态：独立分支、轻量概率模型、影子评分器及[估计／决策分离诊断](DECISION_ABLATION.md)已建立；33 项软件检查通过。[数据接口契约](DATA_INTERFACE.md)进一步明确外层提交点与数域转换边界。训练仅发生在明确标注的人工软件样例上，没有研究数据、原系统实验或吞吐收益结论。[两份独立评审](REVIEW_20260924.md)及精确图诊断促使下一步检查实际组合查询价值，保留概率字典序作对照。运行命令和接口限制见[实现说明](IMPLEMENTATION.md)。
