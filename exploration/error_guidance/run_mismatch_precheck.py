"""Run a frozen public prior under two legal private requester eta conditions.
...
def run(source_root, output):
    ...
    report = {
        ...
        "scope": "frozen_public_prior_private_requester_mismatch",
    ...
    try:
        ...
        report.update(
            ...
            fixture_sha256=sha(HERE / "mismatch_precheck.cpp"),
        ...
        with tempfile.TemporaryDirectory(prefix="mapf_error_mismatch_") as directory:
            ...
            binary = build / "error_guidance_mismatch_precheck"
            compile_command = [
                ...
                str(HERE / "mismatch_precheck.cpp"), "-L", str(library),
...
def main():
    ...
    if args.output.resolve().parent != HERE or not args.output.name.startswith("mismatch_run_"):
        parser.error("output must be mismatch_run_* in this package")
