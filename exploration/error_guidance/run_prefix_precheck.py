"""Run the finite original-prefix ordinary-branch x public nominal-waiting check.
...
def run(source_root, output):
    ...
    report = {
        ...
        "scope": "finite_original_prefix_ordinary_branch_x_public_nominal_waiting",
    ...
    try:
        ...
        report.update(
            ...
            fixture_sha256=sha(HERE / "prefix_precheck.cpp"),
        ...
        with tempfile.TemporaryDirectory(prefix="mapf_error_prefix_") as directory:
            ...
            binary = build / "error_guidance_prefix_precheck"
            compile_command = [
                ...
                str(HERE / "prefix_precheck.cpp"), "-L", str(library),
            ...
            if len([e for e in report["events"] if e["event"] == "factorial_cell"]) != 8:
                raise ValueError("native output lacks eight actual factorial cells")
...
def main():
    ...
    if args.output.resolve().parent != HERE or not args.output.name.startswith("prefix_run_"):
        parser.error("output must be prefix_run_* in this package")
