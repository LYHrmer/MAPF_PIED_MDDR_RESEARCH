"""Compile/run the finite artificial route precheck against read-only pinned headers.
...
def run(source_root, output):
    ...
    report = {
        ...
        "scope": "artificial_finite_offline_reference_potential", "source_root": str(source_root),
        "source_header_sha256": pins, "fixture_sha256": sha(HERE / "precheck.cpp"),
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="mapf_error_guidance_") as directory:
            ...
            binary = build / "error_guidance_precheck"
            compile_command = [
                ...
                str(HERE / "precheck.cpp"), "-L", str(library),
...
def main():
    ...
    if args.output.resolve().parent != HERE or not args.output.name.startswith("precheck_run_"):
        parser.error("output must be precheck_run_* in this package")
