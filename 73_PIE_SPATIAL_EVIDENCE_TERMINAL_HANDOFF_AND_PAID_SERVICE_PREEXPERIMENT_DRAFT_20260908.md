...
点积、加法、叉积比较及有理数编码按实际操作数位长展开；上述多项式计数不是已给出的基本步总费，也不把一次精确谓词标成恒价。首轮容量表只需为所走初始化/源/切分/上述几何/消息/日志/guard和清理逐项登记可暂停工作、不可跨行原子工作、消息字数与发布次数；源中已知n_f k_commit、n_f(k_commit−1)及binom(n_f,2)(k_commit−1)计数继续保留。完整表示/容器操作界尚缺时不得填造单价常数。真实TASK_SERVICE的精确世界求值仍依§9.2计宿主任务设施账，不重新塞入模拟应用CPU行容量；首个真实服务非空性与§13的D相对R/E0净收益是不同义务。
